        ...
        if (command && !command.meta.disable) {
            message.channel.send(
                createEmbed("info")
                    ...
                    .setThumbnail("https://raw.githubusercontent.com/Marshmelly/marshmelly-music/main/.github/images/question_mark.png")
            ...
            message.channel.send(
                createEmbed("info", message.client.commands.filter(cmd => !cmd.meta.disable && cmd.meta.name !== "eval").map(c => `\`${c.meta.name}\``).join(" "))
                    ...
                    .setFooter(`Use ${message.client.config.prefix}help <command> to get information on a specific command.`, "https://raw.githubusercontent.com/Marshmelly/marshmelly-music/main/.github/images/info.png")
            ).catch(e => this.client.logger.error("HELP_CMD_ERR:", e));
        }
    }
}
